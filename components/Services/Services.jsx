...
const Services = () => {
  ...
  const handleDeleteService = async (id) => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_BACKEND_API}/admin/service/${id}`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
    ...
    <div className="w-full max-w-7xl mx-auto p-4 sm:p-6">
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden relative flex flex-col">
        <div className="p-6 border-b border-gray-100 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 bg-white/50 backdrop-blur-xl">
          <div>
            <h2 className="text-2xl font-bold text-gray-800 tracking-tight">Services</h2>
            <p className="text-sm text-gray-500 mt-1">Manage your platform's services and offerings</p>
          </div>
          ...
          <table className="w-full text-left border-collapse whitespace-nowrap">
            <thead>
              <tr className="bg-gray-50/50 text-gray-500 text-[11px] font-bold uppercase tracking-wider border-b border-gray-100">
                ...
                <th className="p-5">Price Plans</th>
            ...
            <tbody className="text-gray-600 text-sm divide-y divide-gray-50">
              {isLoading ? (
                ...
                filteredServices.map((service, index) => (
                  <tr key={service._id || index} className="hover:bg-gray-50/80 transition-colors group cursor-default">
                    <td className="p-5 text-center font-medium text-gray-400">
                      {index + 1 < 10 ? `0${index + 1}` : index + 1}
                    </td>
                    <td className="p-5">
                      <div className="flex items-center gap-4">
                        <div className="w-12 h-12 rounded-xl bg-gray-100 border border-gray-200 overflow-hidden flex-shrink-0 flex items-center justify-center text-2xl">
                          {service.icon ? service.icon : <ImageIcon className="w-5 h-5 text-gray-400" />}
                        </div>
                        <div>
                          <p className="font-semibold text-gray-800 capitalize text-base">{service.name}</p>
                          <p className="text-xs text-gray-500 mt-0.5 max-w-[250px] truncate">{service.shortDesc}</p>
                        </div>
                      </div>
                    </td>
                    <td className="p-5">
                      <span className="font-medium text-gray-700">{service.pricingPlans?.length || 0} Plans</span>
                    </td>
                    <td className="p-5">
                      <span className={`inline-flex items-center px-3 py-1 rounded-full text-[11px] font-bold uppercase tracking-wide border ${service.isPublished ? 'bg-emerald-100 text-emerald-700 border-emerald-200' : 'bg-amber-100 text-amber-700 border-amber-200'}`}>
