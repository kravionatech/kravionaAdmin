...
const Testimonials = () => {
  ...
  const handleDeleteTestimonial = async (id) => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_BACKEND_API}/admin/testimonial/${id}`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
      ...
      toast.error("Network error. Could not delete testimonial.");
    }
  };
    ...
    <div className="w-full max-w-7xl mx-auto p-4 sm:p-6">
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden relative flex flex-col">
        <div className="p-6 border-b border-gray-100 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 bg-white/50 backdrop-blur-xl">
          <div>
            <h2 className="text-2xl font-bold text-gray-800 tracking-tight">Testimonials</h2>
            <p className="text-sm text-gray-500 mt-1">Manage client reviews and testimonials</p>
          </div>
                ...
                <tr>
                  <td colSpan="5" className="p-12 text-center h-64">
                    <div className="flex flex-col items-center justify-center text-gray-400 gap-3">
                      ...
                      <p className="text-base font-medium text-gray-600">No testimonials found</p>
                    </div>
                  </td>
                </tr>
              ) : (
                filteredTestimonials.map((testimonial, index) => (
                  <tr key={testimonial._id || index} className="hover:bg-gray-50/80 transition-colors group cursor-default">
                      ...
                      <div className="flex items-center gap-4">
                        <div className="w-12 h-12 rounded-full bg-gray-100 border border-gray-200 overflow-hidden flex-shrink-0 flex items-center justify-center">
                          {testimonial.clientAvatar ? (
                            <img src={testimonial.clientAvatar} alt={testimonial.clientName} className="w-full h-full object-cover" />
                          ) : (
                        ...
                        <div>
                          <p className="font-semibold text-gray-800 capitalize text-base">{testimonial.clientName}</p>
                          <p className="text-xs text-gray-500 mt-0.5 max-w-[250px] truncate">{testimonial.clientRole} at {testimonial.clientCompany}</p>
                        </div>
                      </div>
                    </td>
                    <td className="p-5">
                      <span className="font-bold text-amber-500">{"★".repeat(testimonial.rating || 5)}{"☆".repeat(5 - (testimonial.rating || 5))}</span>
                    </td>
                    <td className="p-5">
                      <span className={`inline-flex items-center px-3 py-1 rounded-full text-[11px] font-bold uppercase tracking-wide border ${testimonial.isPublished ? 'bg-emerald-100 text-emerald-700 border-emerald-200' : 'bg-amber-100 text-amber-700 border-amber-200'}`}>
                        {testimonial.isPublished ? "Published" : "Draft"}
                      </span>
                    </td>
                    <td className="p-5">
                      <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button onClick={() => handleOpenModal(testimonial)} className="p-2 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-all" title="Edit">
                          <Edit size={16} />
                        </button>
                        <button onClick={() => handleDeleteTestimonial(testimonial._id)} className="p-2 text-gray-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-all" title="Delete">
                          <Trash2 size={16} />
                        </button>
                      </div>
                    </td>
